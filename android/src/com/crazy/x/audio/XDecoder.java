...
public class XDecoder extends Thread {
    ...
    public static interface XDecoderListener {
    ...
    private XDecoderListener mListener = null;

    public XDecoder(XDecoderListener listener) {
    ...
    public void run() {
        ...
        if (mListener != null) {
            mListener.onDecoderStart();
        }
        Log.v(TAG, "Start XDecoder!");
        while (isRunning() || (!isIdle())) {
            ...
            synchronized (mMutex) {
                if (mByteBuffers.size() > 0) {
                    ...
                    if (getSize > 0) {
                        if (mListener != null) {
                            mListener.onDecoderContent(processedData, getSize);
                        }
                    }
                }
            }
        ...
        if (mCacheSize == 0) {
            ...
            if (mCacheSize + size < DECODER_BUFFER_SIZE) {
                ...
                System.arraycopy(data,
                        ...
                        mCacheData,
                        mCacheSize,
                        DECODER_BUFFER_SIZE - mCacheSize);
                ...
                if (mCacheSize > 0) {
                    System.arraycopy(data,
                            offset + DECODER_BUFFER_SIZE * count,
                            mCacheData,
                            0,
                            mCacheSize);
                }
