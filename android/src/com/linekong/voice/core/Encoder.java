        ...
        while (isRunning() || (!isIdle())) {
            ...
            synchronized (mMutex) {
                if (mDataBuffers.size() > 0){
                    ...
                    getSize = mSpeex.encode(db.mData, 0, mProcessedData, db.mSize);
    ...
    public void putData(short[] data, int size){
        if (mCacheSize == 0){
            ...
            for (int i = 0; i < count; i++) {
                ...
                synchronized (mMutex) {
                    System.arraycopy(data, ENCODER_BUFFER_SIZE*i, db.mData, 0, ENCODER_BUFFER_SIZE);
            ...
            if (mCacheSize > 0){
                System.arraycopy(data, ENCODER_BUFFER_SIZE*count, mCacheData, 0, mCacheSize);
            }
        } else {
            if (mCacheSize + size < ENCODER_BUFFER_SIZE){
                ...
                System.arraycopy(data, 0, mCacheData, mCacheSize, ENCODER_BUFFER_SIZE - mCacheSize);
                
                DataBuffer db = new DataBuffer(ENCODER_BUFFER_SIZE);
                synchronized (mMutex) {
                    System.arraycopy(mCacheData, 0, db.mData, 0, ENCODER_BUFFER_SIZE);
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    synchronized (mMutex) {
                        System.arraycopy(data, offset+ENCODER_BUFFER_SIZE*i, db.mData, 0, ENCODER_BUFFER_SIZE);
                ...
                mCacheSize = size%ENCODER_BUFFER_SIZE;
                if (mCacheSize > 0){
                    System.arraycopy(data, offset+ENCODER_BUFFER_SIZE*count, mCacheData, 0, mCacheSize);
                }
            }
        }
    }
